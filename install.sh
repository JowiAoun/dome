...
#   cd ~/.dotfiles
#   ./install.sh --host zenbook-duo     # or --host generic (default)
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      echo "usage: ./install.sh [--host generic|zenbook-duo]"
      exit 0
      ;;
...
if [ ! -f user-config.nix ]; then
  ...
  echo "[dome] created user-config.nix — review name/email/modules before relying on it"
fi
