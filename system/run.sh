...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --brave)             BRAVE_BROWSER=1; export BRAVE_BROWSER; shift ;;
    --no-brave)          BRAVE_BROWSER=0; export BRAVE_BROWSER; shift ;;
    --brave-policy)      BRAVE_POLICY=1; export BRAVE_POLICY; shift ;;
    --no-brave-policy)   BRAVE_POLICY=0; export BRAVE_POLICY; shift ;;
    --gecko-policy)      GECKO_POLICY=1; export GECKO_POLICY; shift ;;
    --no-gecko-policy)   GECKO_POLICY=0; export GECKO_POLICY; shift ;;
...
for script in 05-hostname.sh 10-apt-base.sh 20-kernel.sh 25-memory.sh 30-grub-params.sh 35-boot-services.sh; do
  log "── $script"
  bash "./$script"
done
...
for script in 60-docker.sh 70-docker-desktop.sh 75-claude-desktop.sh 76-openwhispr.sh 77-gecko-policy.sh 78-brave.sh 79-brave-policy.sh 80-nix-gpu.sh 85-apparmor-userns.sh 86-gamemode.sh 87-login-pin.sh 88-faillock.sh 96-tpm-unlock.sh 95-luks.sh; do
  log "── $script"
  bash "./$script"
done
